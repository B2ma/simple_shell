...
/**
  ...
  * @str: the string contsining delimiters
...
int tokenCount(char *str, char *delim)
{
	...
	for (itr = 0; itr < length; itr++)
	{
		...
		{
			words++;
			itr += tokenLength(str + itr, delim);
		}
	}
	return (words);
}
/**
  * strtok_fn - Tokenizes str
  * @str: string to tokenize
  * @delim: delimiters
  * Return: pointer to array of words
  */
char **strtok_fn(char *str, char *delim)
{
	char **words;
	int itr = 0;
	int the_words, a, characters, b;

	the_words = tokenCount(str, delim);
	if (the_words == 0)
		return (NULL);
	...
	for (a = 0; a < the_words; a++)
	{
		while (str[itr] == *delim)
			itr++;
		characters = tokenLength(str + itr, delim);
